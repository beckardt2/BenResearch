...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
 SUBROUTINE HESSIAN(Uene,x,jacobian)

   IMPLICIT NONE
    
    REAL(KIND=r8),INTENT(INOUT) :: Uene
    REAL(KIND=r8),INTENT(IN) :: x(natms2)
    REAL(KIND=r8),INTENT(INOUT)::jacobian(natms2,natms2)
    ...
    DO ix=1,natms-1
      ...
      IF(rijc >= 0.0_r8) THEN
        ...
        !Hessian matrix
          ...
          jacobian(ix,ix)=jacobian(ix,ix)+temp1*rrij*(1.0_r8-xrrij2)+temp2*xrrij2
          jacobian(ix,iy)=jacobian(ix,iy)-temp1*xyij*rrij3+temp2*xyij*rrij2
        
          jacobian(ix,jx)=temp1*rrij*(xrrij2-1.0_r8)-temp2*xrrij2
          jacobian(ix,jy)=temp1*xyij*rrij3-temp2*xyij*rrij2
          jacobian(jx,ix)=jacobian(ix,jx)
          jacobian(jy,ix)=jacobian(ix,jy)
       
          jacobian(iy,iy)=jacobian(iy,iy)+temp1*rrij*(1.0_r8-yrrij2)+temp2*yrrij2
        
          jacobian(iy,jy)=temp1*rrij*(yrrij2-1.0_r8)-temp2*yrrij2
          jacobian(iy,jx)=temp1*xyij*rrij3-temp2*xyij*rrij2  
          jacobian(jy,iy)=jacobian(iy,jy)
          jacobian(jx,iy)=jacobian(iy,jx)
        
          jacobian(jx,jx)=jacobian(jx,jx)+temp1*rrij*(1.0_r8-xrrij2)+temp2*xrrij2
          jacobian(jx,jy)=jacobian(jx,jy)-temp1*xyij*rrij3+temp2*xyij*rrij2
          jacobian(jy,jy)=jacobian(jy,jy)+temp1*rrij*(1.0_r8-yrrij2)+temp2*yrrij2

          jacobian(jy,jx)=jacobian(jx,jy)
        END IF
     END DO            
       jacobian(iy,ix)=jacobian(ix,iy)    
   END DO
...
SUBROUTINE DominantEigen(N,x,lamda)
!Finds the dominant eigenvector and eigenvalue using the power method

Implicit none
Real(KIND=r8), intent(in), dimension(natms2,natms2) :: N
Real(KIND=r8), intent(inout), dimension(natms2)::x
Real(KIND=r8), intent(inout)::lamda
Real(KIND=r8), dimension(natms2)::xi,xf,xc
...
Do While (abs(dot_product(x-xc,x-xc))>0.00001)
        xc=x
        xi=xf
        xf=matmul(N,xi)
        x=xf/maxval(xf)
...
Subroutine LUfactor(N,L,U)

Implicit none
!INTEGER(KIND=int16), intent(in):: dim
Real(KIND=r8), intent(in), dimension(natms2,natms2) :: N
Real(KIND=r8), intent(out), dimension(natms2,natms2) :: L,U
...
subroutine LeastDominant(L,U,vec,val)

implicit none
!INTEGER(KIND=int16), intent(in)::dim
Real(KIND=r8), intent(in), dimension(natms2,natms2) :: L, U
Real(KIND=r8), intent(out), dimension(natms2) :: vec
Real(KIND=r8), intent(out) :: val
Real(KIND=r8), dimension(natms2)::xi,xf,y,d,posx,posy
INTEGER(KIND=int16):: i,j,k
Real(KIND=r8):: mew,v,large 

Do While (abs(dot_product(xf-xi,xf-xi))>0.00000001)
        xi=xf
        Do j=1,natms2,1
                d(j)=xi(j)
                Do k=1,j-1,1
        ...
        End Do
        Do j=0,natms2-1,1
                y(natms2-j)=d(natms2-j)
                Do k=1,j,1
                        y(natms2-j)=y(natms2-j)-y(natms2-k+1)*U(natms2-j,natms2-k+1)
                End Do
        End Do
